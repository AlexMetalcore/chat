...
include ('commentdb.php');
        $commentsql = $connect_db->query("SELECT *FROM comments WHERE id ORDER BY id DESC;");
        	while ($row = $commentsql->fetch_array()) {
				...
				echo '<span class="message right"><br><br>Сообщение:&nbsp;'.$row["comment"].'&nbsp;&nbsp;<br><i>пользователь&nbsp;&nbsp;<b style="color: black;">' .$row["name"]. '</b></a></i><br><b>Дата и время:&nbsp;'.$row["day"]. '&nbsp;в&nbsp;' .$row["commenttime"]. '</b></span>';
			}
                                        }
                        ?>
